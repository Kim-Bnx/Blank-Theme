// TUTORIAL : https://blankthemerpg.forumactif.com/t76-messenger
!window.FAM &&
	(function () {
		"use strict";
		...
		window.FAM = {
			...
			config: {
				...
				lang: {
					...
					not_found_offline:
						'<a href="/login">Connectez-vous</a> ou <a href="/register">incrivez-vous</a> pour voir ce forum.',
					...
					topic_no_message:
						"Vous devez écrire un message avant d'ouvrir votre sujet.",

					no_tabs_title: "Ouvrir un nouvel onglet",
					no_tabs:
						"Oh non ! Vous n'avez pas d'onglet ouvert. Cliquez pour en ouvrir un.",
					...
					login_error:
						"Le nom d'utilisateur ou le mot de passe sont invalides.",
					login_logged_in:
						'Vous êtes déjà  connecté. <a href="javascript:FAM.page.profile.open();">Cliquez ici</a> pour voir votre profil ou <a href="javascript:FAM.history.back(true);">cliquez ici</a> pour retourner à  la page d\'accueil.',
					...
					profile_logged_out:
						'Connectez-vous pour voir votre profil. <a href="javascript:FAM.page.login.open();">Cliquez ici</a> pour vous connecter ou <a href="javascript:FAM.history.back(true);">cliquez ici</a> pour retourner à  la page d\'accueil.',
					profile_my_topics: "Voir vos sujets",
					profile_my_posts: "Voir vos messages",
					profile_mark_read: "Tous marquer comme lus",
					profile_confirm_mark_read:
						"Tous les forums et les sujets seront indiqués comme lus, voulez-vous continuer ?",
					profile_forums_marked:
						"Tous les forums et les sujets sont indiqués comme lus !",
					profile_delete_cookies: "Supprimer les cookies",
					profile_confirm_delete_cookies:
						"Tous les cookies seront supprimés et votre compte sera déconnecté, voulez-vous continuer ?",
					...
					about_new_update:
						"Une nouvelle version de Forumactif Messenger est disponible !",
					about_update_info:
						"Attention! Il s'agit du code de base de AngeTuteur. Il n'est pas adapté à la nouvelle version du Blank Theme.",
					...
					attach_uploading:
						"La fenêtre d'upload est des images est déjà ouverte.",
					attach_searchYT: "Chercher sur Youtube",
					attach_searchingYT: "Youtube est déjà  ouvert.",
					attach_max:
						"Vous avez atteint le nombre maximum de médias à envoyer pour ce message.",
					...
					error_sending:
						"Une erreur empêche l'envoi de votre message. Réessayer ?",
			...
			updateUserdata: function (data) {
				...
				script.text = data.replace(
					/[\s\S]*?(if\(typeof\(_userdata\) == "undefined"\)[\s\S]*?)if\(typeof\(_board\) == "undefined"\)[\s\S]*/,
					"$1"
				);
				...
				FAM.request = $.get(url, function (data) {
					...
					for (; i < j; i++) {
						html +=
							type == "topic"
								? (FAM.message.log[
										a[i].className.replace(/.*?(post--\d+).*/, "$1")
								  ] = FAM.message.parse(a[i], i, a[i + 1]))
								: FAM.topic.parse(a[i]);
					}
					...
					if (type == "topic") {
						...
						if (postReply) {
							...
							FAM.cache.actions.innerHTML =
								...
								FAM.config.lang.msg_placeholder +
								...
								FAM.config.lang.tooltip_send +
								'" type="button"><i class="bi bi-send-fill"></i></button>' +
								'<button id="FAM-attachment" type="button" onclick="FAM.attachments.open(this);" title="' +
								FAM.config.lang.tooltip_attachments +
								'" type="button"><i class="bi bi-file-earmark-arrow-up-fill"></i><span id="FAM-attachment-total">0</span></button>' +
								'<button id="FAM-emoji" type="button" onclick="FAM.message.emoji(this);" title="' +
								FAM.config.lang.tooltip_emoji +
								...
								(form
									? form.outerHTML
											...
											.replace(
												"<form",
												'<form id="FAM-post-data" name="fampost" style="display:none"'
											)
							...
							if (!_userdata.session_logged_in) {
								FAM.message.replyPage = (
									FAM.fVersion == 5 ? postReply : postReply.parentNode
								).href;
						...
						var pagination = /forum|search/.test(type)
							? $(FAM.select.pagination, data)[0]
							: null;
						...
						FAM.cache.content.innerHTML =
							...
							'<div class="FAM-loading FAM-noclick">' +
								...
								FAM.config.lang.not_found +
								(_userdata.session_logged_in
									? ""
									: " " + FAM.config.lang.not_found_offline) +
								"</p>" +
								"</div>";
						FAM.cache.actions.innerHTML = "";
						...
						if (type == "forum") {
							...
							if (newTopic) {
								FAM.cache.actions.innerHTML =
									...
									(FAM.fVersion == 5 ? newTopic : newTopic.parentNode).href +
									'\');" title="' +
									FAM.config.lang.new_topic +
									'">' +
			...
			message: {
				...
				insert: function (string, pad) {
					...
					if (msg) {
						try {
							...
							msg.value =
								msg.value.slice(0, position) +
								addedText +
								msg.value.slice(position, msg.length);
				...
				edit: {
					...
					confirm: function () {
						if (FAM.message.edit.placeholder) {
							FAM.message.backup =
								FAM.message.edit.placeholder.querySelector(
									".FAM-inputbox"
								).value;
					...
					cancel: function () {
						if (FAM.message.edit.placeholder) {
							if (FAM.message.edit.placeholder.parentNode) {
								FAM.message.edit.placeholder.insertAdjacentHTML(
									"afterend",
									FAM.message.edit.backup
								);
				...
				handleKeys: function (e) {
					if (e) {
						...
						if (
							...
							FAM.cache.actions.querySelector("#FAM-send").dataset.disabled !=
								"true" && FAM.message.send();
							e.preventDefault();
						}
					}
				},
				...
				send: function (resend, form, msgData) {
					...
					if (!resend) {
						if (j) {
							for (; i < j; i++) {
								attachments +=
									"[tr][td]" + FAM.attachments.list[i] + "[/td][/tr]";
							}
							...
							FAM.cache.actions.querySelector(
					...
					$.post(
						...
						function (data) {
							...
							if (success) {
								FAM.message.check(function () {
										...
										$.get(success.href, function (data) {
											...
											if (post) {
												placeholder.insertAdjacentHTML(
													"afterend",
													(FAM.message.log[form.p.value] = FAM.message.parse(
														post,
														1,
														""
													))
									...
									FAM.message.timeoutBar = setInterval(function () {
										if ((progress -= 50) <= 0 || !FAM.message.sending) {
											...
											bar.style.height =
												(progress / FAM.config.flood_control) * 100 + "%";
										}
									}, 40);
								...
								FAM.message.error = FAM.message.write(
									'<div class="FAM-center"><i class="fa fa-exclamation-circle fa-2x"></i></div>' +
										captcha
											...
											.replace(
												/<input([^>]*?)style="[^"]*?"([^>]*?)>/,
												'<input class="FAM-inputbox" $1 $2>'
											)
											.replace(
												/<input[^>]*?type="submit"[^>]*?value="(.*?)"[^>]*?>/,
												'<button class="FAM-button" type="button">$1</button>'
											)
								...
								FAM.message.error.querySelector(".FAM-button").onclick =
									function () {
										FAM.message.sending = false;
										FAM.message.send(
											true,
											$(this).closest("form").clone()[0],
											msgData
										);
										return false;
									};
					...
					$.get(page, function (data) {
						...
						for (; i < j; i++) {
							...
							if (!FAM.message.log[pid]) {
								console.log(post[i]);
								html += FAM.message.log[pid] = FAM.message.parse(
									post[i],
									i,
									post[i + 1]
								);
				...
				parse: function (post, index, nextPost) {
					...
					if (FAM.config.ignore_firstpost && index == 0) {
						var indicator = $(
							FAM.select.post_message + " > strong:first-child",
							post
						)[0];
					...
					group = group
						? '<span style="' + group.getAttribute("style") + '"><strong>'
						: null;
					...
					online =
						post.className.indexOf("online") != -1 ||
						(FAM.fVersion == 2 && post.querySelector(".i_icon_online")) ||
						(FAM.fVersion == 0 &&
							post.nextSibling &&
							post.nextSibling.querySelector(".i_icon_online"));
					...
					return (
						...
						(!_userdata.session_logged_in && name == FAM.config.lang.guest)
							? " FAM-my-msg"
							: "") +
						" " +
						...
						(pLink ? pLink : "") +
						'<img src="' +
						(avatar ? avatar.src : FAM.config.no_avatar) +
						'" alt="avatar">' +
						(pLink ? "</a>" : "") +
						"</div>" +
						...
						(date
							? (FAM.fVersion == 5
									? date.innerHTML.split("<").shift()
									: date.innerHTML.split(">").pop()
							  ).replace(/^\s+|\s+$/g, "")
							: "") +
						' ">' +
						'<div class="FAM-msg-content">' +
						...
						(msg
							? msg.innerHTML
									...
									.replace(/class="(.*?)"/g, function (match, $1) {
										return $1.indexOf("FAM-") == -1
											? 'class="FAM-' + $1.split(" ").join(" FAM-") + '"'
											: match;
									})
									...
									.replace(
										/<table class="FAM-attachment">([\s\S]*?)<\/table>/,
										function (match, $1) {
											return (
												'<table class="FAM-attachment">' +
												$1.replace(
													/<td/g,
													'<td onclick="FAM.modal.open(this);"'
												) +
												"</table>"
											);
										}
						...
						(quote
							? '<span class="FAM-msg-button FAM-quote-button" onclick="FAM.message.interact(this, \'' +
							  quote.href +
							  '\');" title="' +
							  FAM.config.lang.tooltip_msg_quote +
						...
						(edit
							? '<span class="FAM-msg-button FAM-edit-button" onclick="FAM.message.interact(this, \'' +
							  edit.href +
							  '\');" title="' +
							  FAM.config.lang.tooltip_msg_edit +
						...
						(remove
							? '<span class="FAM-msg-button FAM-delete-button" onclick="FAM.message.interact(this, \'' +
							  remove.href +
							  '\');" title="' +
							  FAM.config.lang.tooltip_msg_delete +
				...
				check: function (callback) {
					FAM.request = $.get(
						FAM.history.log["tab" + FAM.tab.active][
							FAM.history.log["tab" + FAM.tab.active].length - 1
						].url,
						function (data) {
							for (
								var a = $(
				...
				notify: function (newMsg) {
					...
					if (FAM.message.isVisible(newMsg.previousSibling, 5)) {
						...
						var notif = FAM.cache.chat.querySelector("#FAM-new-msg-notif"),
							notifMsg = FAM.config.lang[
								"message_notif_" +
									(FAM.message.unread.length == 1 ? "singular" : "plural")
							].replace("$N", FAM.message.unread.length);

						// insert the new message notif and divider
						if (!notif) {
							FAM.cache.content.insertAdjacentHTML(
								"beforebegin",
								'<div id="FAM-new-msg-notif" onclick="FAM.message.markRead();">' +
									notifMsg +
									"</div>"
							);
							newMsg.insertAdjacentHTML(
								"beforebegin",
								'<div id="FAM-new-msg-divider">' +
									FAM.config.lang.new_messages +
									"</div>"
							);
						} else {
				...
				read: function (caller) {
					if (
						caller.className.indexOf("FAM-viewing-topic") != -1 &&
						FAM.message.unread.length
					) {
						if (
							caller.scrollTop / (caller.scrollHeight - caller.clientHeight) ==
								1 ||
				...
				listener: {
					...
					start: function () {
						FAM.message.listener.listening = setInterval(
							function () {
								if (!FAM.message.sending) {
									FAM.message.check();
								}
					...
					idle: function () {
						...
						FAM.message.listener.idling = setTimeout(
							function () {
								...
								FAM.cache.chat.insertAdjacentHTML(
									...
									'<div id="FAM-idle" class="FAM-loading" onclick="FAM.message.listener.resume();">' +
										...
										FAM.config.lang.idle +
										"</p>" +
										"</div>"
								);
							},
							FAM.config.timeout > 15 * 60 * 1000
								? 15 * 60 * 1000
								: FAM.config.timeout
						);
					},
				...
				scroll: function (amount) {
					FAM.cache.content.scrollTop =
						amount ||
						(FAM.cache.content.lastChild
							? FAM.cache.content.lastChild.offsetTop - 60
							: 0);
				},
				...
				emoji: function (caller) {
					...
					if (active) {
						active.style.visibility =
							active.style.visibility == "hidden" ? "visible" : "hidden";
					} else {
						...
						selector.onload = function () {
							try {
								var doc = this.contentDocument || this.contentWindow.document,
									emoji = doc.querySelectorAll(
										'a[href*="insert_chatboxsmilie"]'
									),
									close = doc.querySelector(
								...
								for (; i < j; i++) {
									emoji[i].dataset.emoji = emoji[i].href.replace(
										/.*?'(.*?)'.*/,
										"$1"
									);
									emoji[i].href = "#";
								...
								if (select) {
									select.innerHTML =
										select.categ.outerHTML + select.mode.outerHTML;
									select.setAttribute(
										"style",
			...
			attachments: {
				...
				open: function (caller) {
					...
					if (active) {
						...
						attach.innerHTML =
							'<div class="FAM-dropdown-title">' +
							FAM.config.lang.tooltip_attachments +
							"</div>" +
							'<div class="FAM-block-option" onclick="FAM.attachments.addImage();"><span class="FAM-option-icon"><i class="fa fa-camera"></i></span> ' +
							FAM.config.lang.attach_image +
							"</div>" +
							'<div class="FAM-block-option" onclick="FAM.attachments.addGIF();"><span class="FAM-option-icon FAM-text-icon">GIF</span> ' +
							FAM.config.lang.attach_gif +
							"</div>" +
							'<div class="FAM-block-option" onclick="FAM.attachments.addVideo();"><span class="FAM-option-icon"><i class="fa fa-film"></i></span> ' +
							FAM.config.lang.attach_video +
							"</div>" +
							'<div class="FAM-block-option" onclick="FAM.attachments.manage();"><span class="FAM-option-icon"><i class="fa fa-wrench"></i></span> ' +
							FAM.config.lang.attach_remove +
							"</div>";
				...
				back: function () {
					...
					FAM.attachments.open(
						FAM.cache.actions.querySelector("#FAM-attachment")
					);
				},
				...
				add: function (caller, tag) {
					...
					if (value) {
						FAM.attachments.list.push(
							"[" +
								tag +
								"]" +
								(/giphy/.test(value)
									? value.replace("200w.gif", "giphy.gif")
									: value) +
								"[/" +
								tag +
								"]"
						...
						FAM.cache.actions.querySelector("#FAM-attachment-total").innerText =
							FAM.attachments.list.length;
						...
						if (!FAM.cache.actions.querySelector("#FAM-attach-success")) {
							FAM.cache.actions
								.querySelector("#FAM-attach-options")
								.insertAdjacentHTML(
									"beforeend",
									'<div id="FAM-attach-success"><i class="fa fa-check-circle"></i>' +
										FAM.config.lang.attach_success +
										"</div>"
								);

							// remove success message after 300ms
							setTimeout(function () {
								var success = FAM.cache.actions.querySelector(
									"#FAM-attach-success"
								);
				...
				remove: function (caller) {
					...
					if (!parent.children.length) {
						parent.innerHTML =
							'<p class="FAM-center">' + FAM.config.lang.attach_none + "</p>";
					}
				},
					...
					if (thumb.tagName == "IMG") {
						...
						thumb.href =
							(tag == "dailymotion"
								...
								: "") + val;
						thumb.firstChild.className = thumb.firstChild.className.replace(
							/fa-.*/,
							"fa-" +
								(tag == "dailymotion"
									? "film"
				...
				addImage: function () {
					...
					attach.innerHTML =
						...
						'<div class="FAM-dropdown-title"><span class="FAM-toolbar-button FAM-dropdown-back" onclick="FAM.attachments.back();"><i class="bi bi-arrow-left-short"></i></span>' +
						FAM.config.lang.attach_image +
						"</div>" +
						'<div class="FAM-button-input FAM-row">' +
						'<input type="text" class="FAM-inputbox" placeholder="' +
						FAM.config.lang.attach_image +
						'">' +
						...
						"</div>" +
						'<div class="FAM-row">' +
						'<button id="FAM-upload" class="FAM-button-large" onclick="FAM.attachments.upload();" type="button">' +
						FAM.config.lang.attach_upload +
						"</button>" +
						"</div>" +
						"</div>";
				},
				...
				addGIF: function () {
					...
					attach.innerHTML =
						'<div class="FAM-dropdown-inner">' +
						'<div class="FAM-dropdown-title"><span class="FAM-toolbar-button FAM-dropdown-back" onclick="FAM.attachments.back();"><i class="fa fa-arrow-left"></i></span>' +
						FAM.config.lang.attach_gif +
						"</div>" +
						'<div class="FAM-row">' +
						'<input type="text" id="FAM-giphy-search" class="FAM-inputbox" onkeyup="FAM.attachments.giphy.keyHandler(this, event);" placeholder="' +
						FAM.config.lang.giphy_search +
						'">' +
						"</div>" +
						'<div class="FAM-row">' +
				...
				addVideo: function () {
					...
					attach.innerHTML =
						...
						"</div>" +
						'<div class="FAM-row">' +
						'<button id="FAM-youtube" class="FAM-button" onclick="FAM.attachments.openYoutube();" type="button">' +
						FAM.config.lang.attach_searchYT +
						"</button>" +
						"</div>" +
						"</div>";
				},
				...
				manage: function () {
					...
					if (j) {
						for (attachments = ""; i < j; i++) {
							...
							dmotion = /dailymotion/.test(FAM.attachments.list[i])
								? "https://www.dailymotion.com/video/"
								: "";

							attachments +=
								'<div id="FAM-attachment-' +
								i +
								'" class="FAM-row">' +
								(/\[img\]/.test(FAM.attachments.list[i])
									? '<img class="FAM-attachment-thumb" src="' +
									  val.replace("giphy.gif", "100w_s.gif") +
									  '" alt="attachment ' +
									  i +
									  "\" onmouseover=\"this.src=this.src.replace('100w_s.gif', '100w.gif')\" onmouseout=\"this.src=this.src.replace('100w.gif', '100w_s.gif')\">"
									: '<a href="' +
									  dmotion +
									  val +
									  '" class="FAM-attachment-thumb" target="_blank"><i class="fa fa-' +
									  (dmotion
											? "film"
											: /youtube/.test(val)
											? "youtube-play"
											: "external-link") +
									  '"></i></a>') +
								'<input class="FAM-inputbox" type="text" value="' +
								dmotion +
								val +
								'" onkeyup="FAM.attachments.update(this);">' +
				...
				openYoutube: function (caller) {
					...
					FAM.attachments.youtube = window.open(
						"https://www.youtube.com/",
						"",
						"scrollbars=yes,resizable=yes,width=780,height=530"
					);
				},
				...
				upload: function () {
					...
					FAM.attachments.uploader = window.open(
						"https://servimg.com/" + FAM.message.servImgData,
						"",
						"scrollbars=yes,resizable=yes,width=780,height=530"
					);
				},
				...
				giphy: {
					...
					search: function (query) {
						...
						if (query) {
							...
							FAM.attachments.giphy.timeout = setTimeout(function () {
								FAM.attachments.giphy.reset(
									true,
									FAM.config.lang.giphy_searching
								);
								FAM.attachments.giphy.query = encodeURIComponent(query);

								FAM.attachments.giphy.request = $.get(
									"http://api.giphy.com/v1/gifs/search?q=" +
										FAM.attachments.giphy.query +
										"&limit=" +
										FAM.attachments.giphy.limit +
										"&rating=pg-13&api_key=" +
										FAM.attachments.giphy.key,
									function (data) {
										// update global data such as page offsets for scrolling
										FAM.attachments.giphy.request = null;
										FAM.attachments.giphy.offset =
											data.pagination.offset + FAM.attachments.giphy.limit;
										FAM.attachments.giphy.offset_total =
					...
					addGIF: function (data, loadMore) {
						...
						if (j) {
							...
							FAM.attachments.giphy.reset(
								true,
								FAM.config.lang.giphy_not_found
							);
						}
					...
					loadMore: function () {
						if (
							FAM.attachments.giphy.offset < FAM.attachments.giphy.offset_total
						) {
							FAM.attachments.giphy.request = $.get(
								"http://api.giphy.com/v1/gifs/search?q=" +
									FAM.attachments.giphy.query +
					...
					reset: function (resultsOnly, newContent) {
						$("#FAM-giphy-results", FAM.cache.actions).html(
			...
			topic: {
				...
				create: function (url, noHistory) {
					...
					FAM.request = $.get(url, function (data) {
						...
						FAM.cache.content.innerHTML =
							...
							FAM.config.lang.title_placeholder +
							'" name="subject"></div>' +
						...
						if (FAM.topic.backup) {
							...
							if (!FAM.topic.backup.subject) {
								document.fampost.subject.insertAdjacentHTML(
									"afterend",
									'<p class="FAM-error FAM-center">' +
										FAM.config.lang.topic_no_subject +
										"</p>"
								);
							}

							if (!FAM.topic.backup.message) {
								document.fampost.message.insertAdjacentHTML(
									"afterend",
									'<p class="FAM-error FAM-center">' +
										FAM.config.lang.topic_no_message +
										"</p>"
								);
							}
						...
						if (success) {
							FAM.get(
								"/t" +
									...
									"-" +
									encodeURIComponent(
										FAM.topic.backup.subject.toLowerCase().replace(/\s/g, "-")
									) +
									"?view=newest",
				...
				parse: function (forumtitle) {
					if (
						FAM.config.ignore_announcements &&
						$(forumtitle)
							.closest(FAM.select.forum_info)
							.find((FAM.fVersion == 4 ? ".topic-type " : "> ") + " strong")
							.length
					) {
						return "";
					}
					...
					if (!row.find(".AD_LastPA")[0]) {
						...
						var avatar = $(
								FAM.fVersion == 6
									? ".lastpostavatar img"
									: ".lastpostavatar img" || ".lastpostavatar img",
								row
							)[0],
			...
			page: {
				...
				about: {
					open: function (noHistory) {
						...
						FAM.request = $.get(
							FAM.page.dir + "about" + FAM.config.update_channel + ".html",
							function (data) {
								FAM.cache.content.innerHTML = FAM.page
									.parse(data, {
										client_version:
											'<a href="https://github.com/SethClydesdale/forumactif-messenger/releases/tag/' +
											FAM.version +
											'" target="_blank">' +
											FAM.version +
											"</a>",
								...
								var status = FAM.cache.content.querySelector(
										"#FAM-version-status"
									),
									icon = FAM.cache.content.querySelector(
								...
								if (
									FAM.version ==
									FAM.cache.content.querySelector("#FAM-version-github")
										.innerHTML
								) {
					...
					update: function () {
						FAM.cache.content.querySelector("#FAM-update").outerHTML =
							'<p id="FAM-update" style="text-align:center;"><i class="fa fa-circle-o-notch fa-spin fa-2x fa-fw"></i></p>';

						$.get(
							"https://raw.githubusercontent.com/SethClydesdale/forumactif-messenger/master/fam" +
								FAM.config.update_channel +
								".js",
							function (data) {
								var config = data
										.replace(
										...
										.replace(/config.*?:/, "window.fam_new_config = ")
										.replace(/,$/, ";"),
									lang = data
										.replace(/[\s\S]*?(lang.*?:.*?\{[\s\S]*?\})[\s\S]*/, "$1")
										.replace(/lang.*?:/, "window.fam_new_lang = "),
								...
								FAM.cache.content.querySelector("#FAM-update").outerHTML =
									'<div class="FAM-row">' +
									FAM.config.lang.about_update_info +
									"</div>";

								update.value = data.replace(
									/config.*?:.*?\{[\s\S]*?\},/,
									"config : " + JSON.stringify(FAM.config, null, 2) + ","
								);
				...
				settings: {
					...
					open: function (noHistory) {
						...
						FAM.request = $.get(
							FAM.page.dir + "settings.html",
							function (data) {
								var settings =
										window.JSON &&
										window.localStorage &&
										localStorage.fam_settings
											? JSON.parse(localStorage.fam_settings)
											: {},
								...
								FAM.cache.content.innerHTML = FAM.page
									.parse(data, {
										...
										fam_sound_notif:
											typeof settings.fam_sound_notif != "undefined"
												? settings.fam_sound_notif
												: FAM.config.sound_notif_auto_enabled == true
												? "checked"
												: "",
									})
								...
								if (_userdata.user_level == 1) {
									FAM.cache.content.querySelector(
										"#FAM-settings-admin"
									).style.display = "";
								}
							}
					...
					update: function (caller, noCache) {
						...
						switch (id) {
							case "fam_fullscreen":
								if (caller.checked) {
									...
									FAM.cache.chat.className = FAM.cache.chat.className.replace(
										"FAM-fullscreen",
										""
									);
									document.body.style.overflow = "";
							...
							case "fam_window_width":
								FAM.cache.chat.style.width = caller.value
									? caller.value + "%"
									: "";
								break;

							case "fam_window_height":
								FAM.cache.chat.style.height = caller.value
									? caller.value + "%"
									: "";
								break;
						...
						if (!noCache && window.JSON && window.localStorage) {
							var settings = localStorage.fam_settings
								? JSON.parse(localStorage.fam_settings)
								: {};

							settings[id] =
								caller.type == "checkbox"
									? caller.checked
										? "checked"
										: ""
									: caller.value;
					...
					reset: function () {
						if (confirm(FAM.config.lang.settings_default_confirm)) {
							...
							FAM.cache.content.querySelector("#FAM-sound-notif").checked =
								FAM.config.sound_notif_auto_enabled;

							// update the settings
							for (
								var a = FAM.cache.content.querySelectorAll("input, select"),
									i = 0,
									j = a.length;
								i < j;
								i++
						...
						if (
							...
							for (i in settings) {
								switch (i) {
									...
									case "fam_window_width":
										FAM.cache.chat.style.width = settings[i]
											? settings[i] + "%"
											: "";
										break;

									case "fam_window_height":
										FAM.cache.chat.style.height = settings[i]
											? settings[i] + "%"
											: "";
										break;
				...
				config: {
					...
					open: function (noHistory) {
						...
						FAM.request = $.get(FAM.page.dir + "config.html", function (data) {
							...
							// add help icons
							for (
								var a = FAM.cache.content.querySelectorAll("label"),
									i = 0,
									j = a.length,
									id;
								i < j;
								...
								if (id) {
									a[i].nextSibling.insertAdjacentHTML(
										"beforeend",
										'<a class="FAM-icon-link" href="https://github.com/SethClydesdale/forumactif-messenger/wiki/Config#' +
											id.replace("FAM-config-", "") +
							...
							for (i in FAM.config.lang) {
								...
								lang +=
									...
									'<input id="FAM-lang-' +
									i +
									'" class="FAM-inputbox" type="text" value=' +
									q +
									FAM.config.lang[i] +
									q +
									">" +
							...
							FAM.cache.content
								.querySelector("#FAM-config-lang-data")
								.firstChild.insertAdjacentHTML("afterend", lang);
						});
					},

					// imports the selected language file
					importLang: function () {
						var select = FAM.cache.content.querySelector(
								"#FAM-config-lang-import"
							),
							lang = select.value,
							langName = select.querySelector(
								'[value="' + lang + '"]'
							).innerText;
						...
						if (
							lang &&
							confirm(
								FAM.config.lang.config_import_lang_confirm.replace(
									"$LANG",
									langName
								)
							)
						) {
							var button = FAM.cache.content.querySelector(
									"#FAM-config-lang-import-confirm"
								),
							...
							$.get(
								"https://raw.githubusercontent.com/SethClydesdale/forumactif-messenger/master/lang/" +
									lang +
									".js",
								function (data) {
									var script = document.createElement("SCRIPT"),
									...
									if (newAlias) {
										FAM.cache.content
											.querySelector("#FAM-config-lang-data")
											.firstChild.insertAdjacentHTML("afterend", newAlias);
									}
									...
									alert(
										FAM.config.lang.config_import_lang_success.replace(
											"$LANG",
											langName
										)
									);
					...
					exportLang: function () {
						var a = FAM.cache.content.querySelectorAll(
								'#FAM-config-lang-data .FAM-inputbox[id^="FAM-lang-"]'
							),
							textarea = FAM.cache.content.querySelector(
								"#FAM-config-lang-file"
							),
							lang = {},
							i = 0,
							j = a.length;
						...
						FAM.cache.content.querySelector(
							"#FAM-config-lang-success"
						).innerHTML = FAM.config.lang.config_export_lang_success;
					...
					addInitialTab: function (tabList, scroll) {
						...
						var list = FAM.cache.content.querySelector(
								"#FAM-config-initial_tabs"
							),
							tabs = "",
							i = 0,
							j = tabList.length,
							...
							addRow = function (label, value) {
								return (
									'<div class="FAM-row">' +
									'<label class="FAM-label">' +
									label +
									" : </label>" +
									'<div class="FAM-label-value">' +
									'<input class="FAM-inputbox FAM-init-tab-' +
									label +
									'" type="text" value="' +
									(value || "") +
									'">' +
							...
							case "down":
								...
								if (!next) {
									tabList.insertBefore(
										tab,
										tabList.querySelector(".FAM-init-tab")
									);
								} else if (nextNext) {
					...
					update: function () {
						...
						$.get(
							...
							function (data) {
								...
								for (
									var a = FAM.cache.content.querySelectorAll(".FAM-init-tab"),
										i = 0,
										j = a.length;
									i < j;
									i++
								...
								FAM.cache.content.querySelector(
									"#FAM-config-code-success"
								).innerHTML = FAM.config.lang.about_update_info;
								update.value = data.replace(
									/config.*?:.*?\{[\s\S]*?\},/,
									"config : " + JSON.stringify(configClone, null, 2) + ","
								);
								update.style.display = "";
							}
						);
				...
				search: {
					...
					open: function (noHistory) {
						...
						FAM.request = $.get(FAM.page.dir + "search.html", function (data) {
							...
							FAM.page.search.button =
								FAM.cache.content.querySelector("#FAM-search-view");
							FAM.page.search.results = FAM.cache.content.querySelector(
								"#FAM-search-results"
							);
						});
					},
					...
					query: function (str) {
						...
						if (str.length >= 3) {
							FAM.page.search.results.innerHTML =
								'<p class="FAM-center">' +
								FAM.config.lang.search_searching +
								"</p>";
							...
							FAM.page.search.wait = setTimeout(function () {
								...
								FAM.page.search.request = $.get(
									FAM.page.search.data.URI,
									function (data) {
										...
										FAM.page.search.results.innerHTML = html
											? html
											: '<p class="FAM-center">' +
											  FAM.config.lang.search_no_results +
											  "</p>";
									}
								);
							}, 100);
							...
							FAM.page.search.results.innerHTML =
								'<p class="FAM-center">' + FAM.config.lang.search_tip + "</p>";
						}
					},
					...
					get: function () {
						...
						FAM.get(
							FAM.page.search.data.URI,
							FAM.config.lang.tooltip_search + " - " + FAM.page.search.data.key
						);
					},
				...
				login: {
					...
					open: function (noHistory) {
						...
						if (!_userdata.session_logged_in) {
							...
							FAM.cache.content.innerHTML =
								'<div class="FAM-content-block FAM-center"><p>' +
								FAM.config.lang.login_logged_in +
								"</p></div>";
						}
					},
					...
					submit: function () {
						if (!FAM.page.login.submitting) {
							...
							FAM.request = $.post(
								"/login",
								$(document.fam_form_login).serialize() + "&login=1",
								function (data) {
									FAM.page.login.submitting = false;
									...
									if (/_userdata\["session_logged_in"\] = 1/.test(data)) {
										...
										if (!FAM.config.hide_menu_avatar) {
											document.getElementById("FAM-menu-avatar").innerHTML =
												_userdata.avatar;
										}
					...
					toggleMenuActions: function () {
						document.getElementById("FAM-menu-login").style.display =
							_userdata.session_logged_in ? "none" : "";
						document.getElementById("FAM-menu-profile").style.display =
							_userdata.session_logged_in ? "" : "none";
					},
				},
				...
				profile: {
					...
					open: function (noHistory) {
						...
						if (_userdata.session_logged_in) {
							...
							FAM.cache.content.innerHTML =
								'<div class="FAM-content-block FAM-center"><p>' +
								FAM.config.lang.profile_logged_out +
								"</p></div>";
						}
					},
					...
					logout: function () {
						if (
							!FAM.page.profile.logging_out &&
							confirm(FAM.config.lang.profile_confirm_sign_out)
						) {
							...
							FAM.request = $.get(
								"/login?logout=true&change_version=prosilver",
								function (data) {
									...
									if (form) {
										FAM.request = $.get(
											form.action + "&key=" + form.key.value,
											function (data) {
												FAM.updateUserdata(data);
												FAM.page.profile.logging_out = false;
												FAM.page.login.open();
										...
										// otherwise we should send them to the logout page directly
						...
						if (confirm(FAM.config.lang["profile_confirm_" + action])) {
							...
							$.get(caller.href, function (data) {
								switch (action) {
									...
									case "delete_cookies":
										var form = $(
											'form[action="/?mode=delete_cookies"]',
											data
										)[0];
			...
			history: {
					...
					if (window.JSON && window.localStorage && localStorage.fam_data) {
						if (!FAM.tab.loaded) {
							...
							for (i in FAM.history.log) {
								FAM.tab.add(
									i.replace("tab", ""),
									FAM.history.log[i][FAM.history.log[i].length - 1].title
								);
							}
				...
				back: function (begin) {
					var history =
						FAM.history.log["tab" + FAM.tab.active][
							begin ? 0 : FAM.history.log["tab" + FAM.tab.active].length - 2
						];

					begin
						? (FAM.history.log["tab" + FAM.tab.active] = FAM.history.log[
								"tab" + FAM.tab.active
						  ].slice(0, 1))
			...
			tab: {
				...
				initial: function () {
					...
					for (
						var i = 0, j = FAM.config.initial_tabs.length, id, title;
						i < j;
						i++
					) {
						...
						FAM.config.initial_tabs[i].url =
							FAM.config.initial_tabs[i].url || "";
						FAM.config.initial_tabs[i].title =
							FAM.config.initial_tabs[i].title || "Tab " + id;
				...
				add: function (restore, title) {
					...
					FAM.cache.tabs.insertAdjacentHTML(
						...
						'<div id="FAM-tab-' +
							...
							id +
							')">' +
							(title ? title : FAM.config.lang.loading) +
							"</span>" +
				...
				close: function (id) {
					var dead = FAM.cache.tabs.querySelector("#FAM-tab-" + id),
						survivor = dead.previousSibling
							? dead.previousSibling
							: dead.nextSibling
							? dead.nextSibling
							: null;
					...
					if (survivor && id == FAM.tab.active) {
						...
						FAM.cache.content.innerHTML =
							...
							'<p><i class="fa fa-plus fa-3x"></i>' +
							FAM.config.lang.no_tabs +
							"</p>" +
							"</div>";
						FAM.cache.actions.innerHTML = "";
				...
				focus: function (id) {
					...
					var active = FAM.cache.tabs.querySelector(
					...
					FAM.cache.tabs.scrollLeft =
						tab.offsetLeft - tab.getBoundingClientRect().width;
					FAM.tab.active = id;
				...
				prompt: function (caller) {
					if (FAM.config.initial_tabs.length) {
						...
						caller.innerHTML =
							'<div class="FAM-row">' +
							'<p><i class="fa fa-question-circle fa-3x"></i>' +
							FAM.config.lang.no_tabs_initial +
							"</p>" +
				...
				title: function (string) {
					...
					FAM.cache.tabs.querySelector(
						"#FAM-tab-" + FAM.tab.active + " .FAM-tab-name"
					).innerText = string;
				},
			},
			...
			modal: {
				...
				open: function (caller) {
					...
					if (attachments) {
						for (
							var a = attachments.querySelectorAll("img, iframe, a"),
								i = 0,
								j = a.length;
							i < j;
							i++
				...
				showMedia: function (id, arrow) {
					...
					FAM.modal.view.href =
						FAM.modal.media[FAM.modal.mediaIndex][
							/IMG|IFRAME/.test(FAM.modal.media[FAM.modal.mediaIndex].tagName)
								? "src"
								: "href"
						];
			...
			init: function () {
				...
				if (
					/\/t\d+/.test(FAM.config.chat_page) &&
					!/view=newest/.test(FAM.config.chat_page)
				) {
					FAM.config.chat_page =
						FAM.config.chat_page.replace(/#\d+$/, "") + "?view=newest";
				}

				// parse the update channel
				FAM.config.update_channel =
					FAM.config.update_channel == "developer" ? "-dev" : "";
				...
				function build() {
					...
					var button = document.createElement("A"),
						...
						embed = FAM.config.embed
							? document.querySelector(FAM.config.embed)
							: null;
					...
					chat.innerHTML =
						...
						FAM.config.lang.tooltip_back +
						'"><i class="bi bi-arrow-left-short"></i></span>' +
						'<h1 class="FAM-maintitle"></h1>' +
						'<span id="FAM-menu-toggle" class="FAM-toolbar-button" title="' +
						FAM.config.lang.tooltip_menu +
						'"><i class="bi bi-three-dots"></i></span>' +
						'<div id="FAM-menu" style="visibility:hidden;">' +
						'<div id="FAM-menu-login" class="FAM-menu-option" onclick="FAM.page.login.open();" title="' +
						FAM.config.lang.tooltip_login +
						'"' +
						(_userdata.session_logged_in ? ' style="display:none"' : "") +
						'><i class="fa fa-sign-in"></i></div>' +
						'<div id="FAM-menu-profile" class="FAM-menu-option" onclick="FAM.page.profile.open();" title="' +
						FAM.config.lang.tooltip_profile +
						'"' +
						(!_userdata.session_logged_in ? ' style="display:none"' : "") +
						">" +
						(!FAM.config.hide_menu_avatar
							? '<span id="FAM-menu-avatar">' + _userdata.avatar + "</span>"
							: '<i class="fa fa-user"></i>') +
						"</div>" +
						'<div id="FAM-menu-search" class="FAM-menu-option" onclick="FAM.page.search.open();" title="' +
						FAM.config.lang.tooltip_search +
						'"><i class="bi bi-search"></i></div>' +
						'<div id="FAM-menu-about" class="FAM-menu-option" onclick="FAM.page.about.open();" title="' +
						FAM.config.lang.tooltip_about +
						'"><i class="bi bi-question-circle-fill"></i></div>' +
						"</div>" +
						"</div>" +
						"</div>" +
					...
					FAM.select = {
						forumtitle:
							FAM.fVersion == 6
								? '.forum-description h3 a[href^="/f"], .posts-description h3 a[href^="/t"]'
								: "a.forum" +
								  (FAM.fVersion == 0 ? "link" : "title") +
								  ", a.topictitle",
						post: ".post_row",
					...
					$(document).on("mousedown", function (e) {
						...
						var close = [
								[
									FAM.cache.toolbar.querySelector("#FAM-menu"),
									FAM.cache.toolbar.querySelector("#FAM-menu-toggle"),
								],
								[
									FAM.cache.actions.querySelector("#FAM-emoji-list"),
									FAM.cache.actions.querySelector("#FAM-emoji"),
								],
								[
									FAM.cache.actions.querySelector("#FAM-attach-options"),
									FAM.cache.actions.querySelector("#FAM-attachment"),
								],
							],
							i = 0,
